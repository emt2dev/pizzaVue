Vue.createApp({
    ...
    methods: {
        addPizza(id) {
            // requires id argument so we know which to add to the cart
        ...
        reduceItem(id) {
            ...
            if (item.quantity == 0) {
                this.cart = this.cart.filter((item) => item.id != id); // removes the id
            } else {
                item.quantity--;
            }
        },
        ...
        removeItem(id) {
            this.cart = this.cart.filter((item) => item.id != id); // removes the id from the array
            item.quantity = 0;
        }
    }
